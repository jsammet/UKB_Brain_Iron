...
def tester(model, dataset, indices, params,criterion):
    ...
    str(params['nb_epochs']) + "_eps_" + str(params['class_nb'])+'_class_'+ str(params['lr'])+"_lr_" +params['activation_type']+"_seed_"+str(params['seed'])+".csv", sep=',',index=False)

    # calc class-wise and overall test accuracy and export to file
    sz_ = np.zeros(params['class_nb'])
    tp_full, fp_full, tn_full, fn_full, sens_cl, spec_cl = sz_, sz_, sz_, sz_, sz_, sz_
    for i in range(params['class_nb']):
        tp_ = fp_ = tn_ = fn_ = 0
        ...
        sens_cl[i] <- tp_ / (tp_ + fn_)
        spec_cl[i] <- tn_ / (tn_ + fp_)
    sens_full = tp_full / (tp_full + fn_full)
    spec_full = tn_full / (tn_full + fp_full)
    acc_ = tp_full / len(test_true)
